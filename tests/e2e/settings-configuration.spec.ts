...
test.describe('Settings Configuration', () => {
  ...
  test('should show LLM provider options', async ({ page }) => {
    const providerSelect = page.locator('select[name="llmProvider"], [data-testid="llm-provider-select"]');
    ...
    if (await providerSelect.isVisible()) {
      ...
      await expect(page.locator('option:has-text("OpenAI"), text=OpenAI')).toBeVisible();
      await expect(page.locator('option:has-text("Anthropic"), text=Anthropic')).toBeVisible();
      await expect(page.locator('option:has-text("Google"), text=Google')).toBeVisible();
    }
  });

  test('should handle API key configuration', async ({ page }) => {
    const apiKeyInput = page.locator('input[type="password"][placeholder*="API"], input[data-testid="api-key-input"]');
    ...
    if (await apiKeyInput.isVisible()) {
      ...
      const toggleButton = page.locator('button[data-testid="toggle-api-key-visibility"]');
  ...
  test('should configure model selection', async ({ page }) => {
    ...
    if (await modelSelect.isVisible()) {
      ...
      await expect(page.locator('option:has-text("gpt-4"), text=gpt-4')).toBeVisible();
      await expect(page.locator('option:has-text("gpt-3.5"), text=gpt-3.5')).toBeVisible();
    }
  });

  test('should adjust temperature setting', async ({ page }) => {
    const temperatureSlider = page.locator('input[type="range"][name*="temperature"], [data-testid="temperature-slider"]');
    const temperatureInput = page.locator('input[type="number"][name*="temperature"], [data-testid="temperature-input"]');
  ...
  test('should configure system prompt', async ({ page }) => {
    const systemPromptTextarea = page.locator('textarea[name*="system"], textarea[placeholder*="system"], [data-testid="system-prompt"]');
    
    if (await systemPromptTextarea.isVisible()) {
      const customPrompt = 'You are a helpful AI assistant specialized in document analysis.';
  ...
  test('should handle RAG settings', async ({ page }) => {
    ...
    const embeddingSelect = page.locator('select[name*="embedding"], [data-testid="embedding-select"]');
    
    if (await embeddingSelect.isVisible()) {
      await embeddingSelect.click();
      await expect(page.locator('option:has-text("text-embedding"), text=embedding')).toBeVisible();
    }
    
    // Check for context length setting
    const contextLengthInput = page.locator('input[name*="context"], input[name*="length"], [data-testid="context-length"]');
  ...
  test('should save settings', async ({ page }) => {
    ...
    if (await saveButton.isVisible()) {
      ...
      const successMessage = page.locator('text=saved successfully, text=settings updated, [data-testid="success-message"]');
  ...
  test('should validate required fields', async ({ page }) => {
    ...
    if (await saveButton.isVisible()) {
      ...
      const apiKeyInput = page.locator('input[type="password"][placeholder*="API"]');
      if (await apiKeyInput.isVisible()) {
        ...
        const errorMessage = page.locator('text=required, text=cannot be empty, [data-testid="error-message"]');
  ...
  test('should reset settings', async ({ page }) => {
    const resetButton = page.locator('button:has-text("Reset"), button:has-text("Defaults")');
    ...
    if (await resetButton.isVisible()) {
      ...
      const confirmDialog = page.locator('text=Are you sure, text=reset to default');
      if (await confirmDialog.isVisible()) {
        const confirmButton = page.locator('button:has-text("Confirm"), button:has-text("Reset")');
  ...
  test('should handle settings import/export', async ({ page }) => {
    ...
    if (await importButton.isVisible()) {
      ...
      await importButton.setInputFiles({
        ...
        buffer: Buffer.from(JSON.stringify({ llmProvider: 'openai', temperature: 0.7 }))
      });
      
      // Should process the import
      const successMessage = page.locator('text=imported successfully, text=settings loaded');
