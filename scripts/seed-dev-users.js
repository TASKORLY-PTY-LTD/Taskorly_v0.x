...
async function createTenantConfiguration(tenantId) {
  ...
  const { error } = await supabase.from('tenant_configurations').insert({
    tenant_id: tenantId,
    llm_provider: 'openai',
    llm_model: 'gpt-4o',
    ...
    system_prompt:
      'You are a helpful AI assistant for Taskorly development testing.',
    temperature: 0.7,
...
async function seedDevUsers() {
  ...
  try {
    ...
    TEST_USERS.forEach(user => {
      console.log(
        `│ ${user.role.toUpperCase().padEnd(8)} │ ${user.email.padEnd(20)} │ ${user.password.padEnd(15)} │`
      );
    });
    ...
    console.log(
      `│ TENANT   │ ${tenant.name.padEnd(20)} │ ID: ${tenant.id.substring(0, 8)}... │`
    );
    console.log(
      '└─────────────────────────────────────────────────────────────┘\n'
    );
    ...
    console.log(
      '  2. Use admin@taskorly.dev to test admin features (no billing access)'
    );
    console.log(
      '  3. Use manager@taskorly.dev to test document/chat management'
    );
    console.log('  4. Use user@taskorly.dev to test basic user functionality');
    console.log('');
    console.log(
      '⚠️  These accounts are for DEVELOPMENT ONLY and should not be used in production!'
    );
  } catch (error) {
