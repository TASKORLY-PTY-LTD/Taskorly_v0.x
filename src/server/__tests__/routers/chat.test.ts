...
describe('Chat Router', () => {
          ...
          update: vi.fn(() => ({
            eq: vi.fn(() => ({
              eq: vi.fn(() => ({
                eq: vi.fn(() => ({
                  select: vi.fn(() => ({
                    single: vi.fn(() => ({ data: { id: 'conv-1' }, error: null }))
                  }))
                }))
              }))
            }))
  ...
  describe('sendMessage', () => {
    ...
    it('should throw error when conversation not found', async () => {
      ...
      mockContext.supabaseAdmin.from().select().eq().eq().single.mockReturnValueOnce({
        data: null,
  ...
  describe('createConversation', () => {
    it('should create new conversation', async () => {
      mockContext.supabaseAdmin.from().insert().select().single.mockReturnValueOnce({
        data: {
          id: 'new-conv',
    ...
    it('should handle creation errors', async () => {
      ...
      await expect(caller.createConversation({
        title: 'New Conversation'
  ...
  describe('listConversations', () => {
    it('should list user conversations', async () => {
      mockContext.supabaseAdmin.from().select().eq().eq().order().range.mockReturnValueOnce({
        data: [
          {
            id: 'conv-1',
  ...
  describe('searchDocuments', () => {
    it('should search documents with RAG pipeline', async () => {
      // Mock tenant configuration
      mockContext.supabaseAdmin.from().select().eq().single.mockReturnValueOnce({
        data: {
          llm_provider: 'openai',
          llm_model: 'gpt-4',
          tenant_id: 'tenant-1'
