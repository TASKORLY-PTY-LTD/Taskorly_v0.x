...
export const configRouter = createTRPCRouter({
  ...
  getUsageStats: tenantProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const totalCost =
        stats?.reduce((sum, log) => sum + (log.cost_cents || 0), 0) || 0;
      const totalMessages =
        stats?.filter(log => log.event_type === 'message').length || 0;
      const totalDocuments =
        stats?.filter(log => log.event_type === 'document_upload').length || 0;
      const totalToolCalls =
        stats?.filter(log => log.event_type === 'tool_execution').length || 0;
