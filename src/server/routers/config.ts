...
//   updateConfig: adminTenantProcedure
//     .input(
//       z.object({
...
//         if (input.llm_model !== undefined)
//           updateData.llm_model = input.llm_model;
//         if (input.embedding_model !== undefined)
//           updateData.embedding_model = input.embedding_model;
//         if (input.temperature !== undefined)
//           updateData.temperature = input.temperature;
//         if (input.max_context_length !== undefined)
//           updateData.max_context_length = input.max_context_length;
//         if (input.system_prompt !== undefined)
//           updateData.system_prompt = input.system_prompt;
//         if (input.vector_db_config !== undefined)
//           updateData.vector_db_config = input.vector_db_config;
...
//           updateData.embedding_api_key = input.embedding_api_key
//             ? encrypt(input.embedding_api_key)
//             : null;
//         }
...
//               embedding_api_key: input.embedding_api_key
//                 ? encrypt(input.embedding_api_key)
//                 : null,
...
//               system_prompt:
//                 input.system_prompt ||
//                 'You are a helpful AI assistant that can access and search through documents to provide accurate, relevant responses.',
...
//   getDecryptedApiKey: tenantProcedure
//     .input(
//       z.object({
...
//   testConfig: adminTenantProcedure.mutation(async ({ ctx }) => {
//     try {
//       // Get configuration
//       const { data: config, error } = await ctx.supabaseAdmin
//         .from('tenant_configurations')
...
//         const embeddingApiKey = decrypt(config.llm_api_key);
//         try {
...
//       const totalTokens =
//         stats?.reduce((sum, log) => sum + (log.tokens_used || 0), 0) || 0;
//       const totalCost =
//         stats?.reduce((sum, log) => sum + (log.cost_usd || 0), 0) || 0;
//       const totalMessages =
//         stats?.filter(log => log.operation === 'message').length || 0;
//       const totalDocuments =
//         stats?.filter(log => log.operation === 'document_upload').length || 0;
//       const totalToolCalls =
//         stats?.filter(log => log.operation === 'tool_execution').length || 0;
...
//               if (log.operation === 'message') acc[date].messages++;
//               if (log.operation === 'document_upload') acc[date].documents++;
//               if (log.operation === 'tool_execution') acc[date].tools++;
//             }
//             return acc;
//           },
