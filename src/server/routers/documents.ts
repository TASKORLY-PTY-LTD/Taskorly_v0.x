    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            `Failed to load PDF parsing library: ${importError instanceof Error ? importError.message : 'Unknown error'}`
          );
        }
        ...
        if (!content || content.length < 10) {
          throw new Error(
            'No text content could be extracted from this PDF. The PDF might be image-based or corrupted.'
          );
        }
      ...
      try {
        ...
        if (
          ...
          throw new TRPCError({
            code: 'BAD_REQUEST',
            message:
              'Document has already been processed. Delete existing chunks first to reprocess.',
          });
        }
        ...
        const pageCount = isPDF
          ? (document.metadata as any)?.pageCount || 0
          : null;

        await logger.info(
          `Starting processing for document: ${document.title}`,
          {
            documentId: document.id,
        ...
        try {
          ...
          await logger.info(
            `Generated ${chunks.length} chunks for document: ${document.title}`,
            {
              documentId: document.id,
          ...
          try {
            ...
            if (vectorProcessingResult.success) {
              await logger.info(
                `Successfully processed vectors for document: ${document.title}`,
                {
                  documentId: document.id,
                  totalChunks: vectorProcessingResult.totalChunks,
                  successfulEmbeddings:
                    vectorProcessingResult.successfulEmbeddings,
              ...
              await logger.warn(
                `Vector processing completed with issues for document: ${document.title}`,
                {
                  documentId: document.id,
            ...
            await logger.error(
              `Vector processing failed for document: ${document.title}`,
              {
                documentId: document.id,
                error: errorMessage,
              }
          ...
          // STEP 5: Log usage
          const chunkingTokens = chunks.reduce(
            (sum: number, chunk: DocumentChunk) =>
              sum + Math.ceil(chunk.content.length / 4),
            0
          );
          ...
          await logger.info(
            `Successfully processed document: ${document.title}`,
            {
              documentId: document.id,
              chunkCount: chunks.length,
              totalTokens,
          ...
          return {
            ...
            errors:
              vectorProcessingErrors.length > 0
          ...
          await logger.error(
            `Document processing failed for: ${document.title}`,
            {
              documentId: document.id,
              isPDF,
              error:
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                if (vectorProcessingResult.success) {
                  await logger.info(
                    `Successfully processed vectors for bulk document: ${doc.title}`,
                    {
                      documentId: document.id,
                      totalChunks: vectorProcessingResult.totalChunks,
                      successfulEmbeddings:
                        vectorProcessingResult.successfulEmbeddings,
                      successfulStorages:
                        vectorProcessingResult.successfulStorages,
                      bulkOperation: true,
                    }
                  );
                } else {
                  await logger.warn(
                    `Vector processing completed with issues for bulk document: ${doc.title}`,
                    {
                      documentId: document.id,
                ...
                await logger.error(
                  `Vector processing failed for bulk document: ${doc.title}`,
                  {
                    documentId: document.id,
                    chunkCount: chunks.length,
                    error: errorMessage,
              ...
              results.push({
                ...
                chunkingTokens,
                embeddingTokens,
                vectorProcessingSuccess:
                  vectorProcessingResult?.success || false,
                vectorProcessingErrors,
              });

              // Log successful processing of individual document in bulk operation
              await logger.info(
                `Successfully processed bulk document: ${doc.title} with ${chunks.length} chunks`,
                {
                  documentId: document.id,
                  chunkCount: chunks.length,
                  tokens: docTokens,
              ...
              const errorMessage =
                chunkingError instanceof TRPCError
              ...
              await logger.error(
                `Bulk document chunking failed for: ${doc.title}`,
                {
                  documentId: document.id,
                  error: errorMessage,
                  bulkOperation: true,
            ...
            await logger.error(
              `Bulk document processing failed for: ${doc.title}`,
              {
                error: errorMessage,
                bulkOperation: true,
              }
        ...
        if (totalTokens > 0) {
          ...
          const vectorProcessingSuccessCount = successfulResults.filter(
            r => r.vectorProcessingSuccess
          ).length;
          const totalVectorProcessingErrors = successfulResults.reduce(
            (sum, r) => sum + (r.vectorProcessingErrors?.length || 0),
            0
          );
          ...
          await ctx.supabaseAdmin.from('usage_logs').insert({
            ...
            metadata: {
              ...
              vector_processing_failure_count:
                successfulResults.length - vectorProcessingSuccessCount,
              total_vector_processing_errors: totalVectorProcessingErrors,
              average_tokens_per_document: Math.round(
                totalTokens / input.documents.length
              ),
            },
          });
        }
        ...
        await logger.info(
          `Bulk upload completed: ${successfulCount}/${input.documents.length} successful`,
          {
            ...
            successRate: Math.round(
              (successfulCount / input.documents.length) * 100
            ),
          }
        );
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        try {
          ...
          await logger.info(
            `Successfully deleted vectors for document: ${input.documentId}`,
            {
              documentId: input.documentId,
          ...
          await logger.warn(
            `Vector cleanup failed for document: ${input.documentId}`,
            {
              ...
              chunkCount: document.chunk_count || 0,
              error:
                vectorCleanupError instanceof Error
                  ? vectorCleanupError.message
                  : 'Unknown vector cleanup error',
            }
          );
        }
