...
export async function generateEmbeddings(
  ...
  try {
    ...
    await logger.info(`Starting vector embedding generation for ${chunks.length} chunks`, {
      documentId,
    ...
    for (let i = 0; i < chunks.length; i += embeddingConfig.batchSize) {
      ...
      try {
        ...
        const batchResults = await processBatch(batch, documentId, tenantId, embeddingConfig, genAI);
        results.push(...batchResults);
        
        // Log progress for monitoring
        await logger.info(`Processed embedding batch ${Math.floor(i / embeddingConfig.batchSize) + 1}`, {
          documentId,
          batchSize: batch.length,
          totalProcessed: Math.min(i + embeddingConfig.batchSize, chunks.length),
        ...
        await logger.error(`Failed to process embedding batch ${Math.floor(i / embeddingConfig.batchSize) + 1}`, {
          documentId,
          batchSize: batch.length,
    ...
    await logger.info(`Successfully generated ${results.length} vector embeddings`, {
      documentId,
      totalChunks: chunks.length,
    ...
    await logger.error(`Vector embedding generation failed for document: ${documentId}`, {
      documentId,
      chunkCount: chunks.length,
    ...
    throw new Error(`Failed to generate vector embeddings: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
async function processBatch(
  ...
  try {
    ...
    throw new Error(`Batch processing failed: ${error instanceof Error ? error.message : 'Unknown batch error'}`);
  }
}
