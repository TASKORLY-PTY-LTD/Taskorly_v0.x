...
function initializePineconeClient(): Pinecone {
  if (!env.PINECONE_API_KEY) {
    throw new Error(
      'Pinecone API key is not configured. Please set PINECONE_API_KEY in your environment variables.'
    );
  }

  if (!env.PINECONE_INDEX_NAME) {
    throw new Error(
      'Pinecone index name is not configured. Please set PINECONE_INDEX_NAME in your environment variables.'
    );
  }
...
export async function storeEmbeddings(
  ...
  try {
    ...
    await logger.info(
      `Starting vector storage for ${embeddings.length} embeddings`,
      {
    ...
    for (let i = 0; i < vectors.length; i += batchSize) {
      ...
      try {
        ...
        // Log batch progress
        await logger.info(
          `Upserted vector batch ${Math.floor(i / batchSize) + 1}`,
          {
        ...
        await logger.error(
          `Failed to upsert vector batch ${Math.floor(i / batchSize) + 1}`,
          {
            batchSize: batch.length,
            error:
    ...
    // Log completion
    await logger.info(
      `Vector storage completed: ${upsertedCount} successful, ${failedCount} failed`,
      {
...
export async function searchSimilarVectors(
  ...
  try {
    ...
    // Log search results
    await logger.info(
      `Vector search completed: ${results.length} results found`,
      {
    ...
    throw new Error(
      `Failed to search vectors in Pinecone: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
...
export async function deleteVectors(
  ...
  try {
    ...
    await logger.info(
      `Starting individual vector deletion for ${ids.length} vectors`,
      {
    ...
    await logger.info(
      `Completed individual deletions: ${deletedCount}/${ids.length} successful`,
      {
...
export async function getVectorStats(
  ...
  try {
    ...
    throw new Error(
      `Failed to get vector statistics: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
