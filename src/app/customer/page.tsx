...
const MOCK_SUGGESTIONS: Suggestion[] = [
  { id: '1', text: 'How do I process a refund in Square?', category: 'pos', icon: <Square className="h-4 w-4" /> },
  { id: '2', text: 'Payment terminal not responding', category: 'troubleshoot', icon: <Shield className="h-4 w-4" /> },
  { id: '3', text: 'How to add a new product?', category: 'pos', icon: <Sparkles className="h-4 w-4" /> },
  { id: '4', text: 'Generate daily sales report', category: 'general', icon: <Monitor className="h-4 w-4" /> },
];
...
export default function CustomerChatPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: 'Hi! I\'m your AI assistant for POS systems. I can help you with Square, troubleshooting, or any questions about your system. What can I help you with today?',
      timestamp: new Date(),
    }
  ]);
  ...
  const generateMockResponse = (userInput: string, context: ScreenContext): string => {
    ...
    } else if (userInput.toLowerCase().includes('payment') || userInput.toLowerCase().includes('terminal')) {
    ...
    } else if (userInput.toLowerCase().includes('product') || userInput.toLowerCase().includes('add')) {
          ...
          <div className="flex items-center space-x-3">
            {screenContext.posSystem && (
              <Badge variant="outline" className="border-blue-400 text-blue-400">
                ...
                {screenContext.posSystem.charAt(0).toUpperCase() + screenContext.posSystem.slice(1)} Connected
              </Badge>
            )}
              ...
              <div className="p-4">
                ...
                <div className="space-y-3 text-sm">
                  <div>
                    ...
                    <span className="ml-2 text-white capitalize">{screenContext.posSystem}</span>
                  </div>
                  
                  <div>
                    <span className="text-slate-400">Current Page:</span>
                    <span className="ml-2 text-white">{screenContext.currentScreen}</span>
                  </div>
                  
                  {screenContext.url && (
                    <div>
                      <span className="text-slate-400">URL:</span>
                      <span className="ml-2 text-blue-400 text-xs break-all">{screenContext.url}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-700/50">
                  ...
                  <div className="space-y-2">
                    <Button variant="outline" size="sm" className="w-full justify-start border-slate-600 hover:bg-slate-700/50 text-slate-300">
