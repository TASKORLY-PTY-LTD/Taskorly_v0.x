...
export default function DashboardPage() {
  const { mockMessages, mockDocuments, mockMCPServers, isDevMode } = useDevMode();

  const totalMessages = mockMessages.length;
  const totalDocuments = mockDocuments.length;
  const readyDocuments = mockDocuments.filter(doc => doc.status === "ready").length;
    ...
    <div className="flex-1 space-y-4 p-4 md:p-6 pt-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Welcome to your RAG Chat System. {isDevMode && "Running in development mode with mock data."}
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{connectedServers}/{mockMCPServers.length}</div>
            <p className="text-xs text-muted-foreground">
              Connected servers
            </p>
          </CardContent>
            ...
            {recentDocuments.map((document) => (
              <div key={document.id} className="flex items-start space-x-3">
                <div className="mt-1">
                  ...
                  {document.status === "processing" && <Clock className="h-4 w-4 text-yellow-500" />}
                  {document.status === "error" && <XCircle className="h-4 w-4 text-red-500" />}
                </div>
                <div className="flex-1 min-w-0">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <Button variant="outline" className="justify-start h-auto p-4">
              ...
              <div className="text-left">
                ...
                <div className="text-sm text-muted-foreground">Begin a new conversation</div>
              </div>
            </Button>
            ...
            <Button variant="outline" className="justify-start h-auto p-4">
              ...
              <div className="text-left">
                ...
                <div className="text-sm text-muted-foreground">Add files to knowledge base</div>
              </div>
            </Button>
            ...
            <Button variant="outline" className="justify-start h-auto p-4">
              ...
              <div className="text-left">
                ...
                <div className="text-sm text-muted-foreground">Check MCP connections</div>
              </div>
            </Button>
          </div>
        </CardContent>
