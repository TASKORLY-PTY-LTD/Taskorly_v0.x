  ...
  if (message.isStreaming) {
    return (
      <div className="flex justify-start">
        <div className={`flex items-start space-x-3 ${isOverlay ? 'space-x-2' : ''}`}>
          <div className={`rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center ${
            ...
            <Bot className={`text-white ${isOverlay ? 'w-3 h-3' : 'w-4 h-4'}`} />
          </div>
          <div className={`rounded-2xl px-4 py-3 bg-slate-800/60 backdrop-blur-lg border border-slate-700/50 ${
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className={`text-slate-400 ${isOverlay ? 'text-xs' : 'text-sm'}`}>
                AI is thinking...
              </span>
            </div>
          </div>
    ...
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div className={`flex items-start max-w-2xl ${
        ...
        <div className={`rounded-full flex items-center justify-center ${
          ...
          {isUser ? (
            <User className={`text-white ${isOverlay ? 'w-3 h-3' : 'w-4 h-4'}`} />
          ) : (
            <Bot className={`text-white ${isOverlay ? 'w-3 h-3' : 'w-4 h-4'}`} />
          )}
        </div>
        ...
        <div className="space-y-2">
          {/* Main message bubble */}
          <div className={`${isOverlay ? 'rounded-lg px-3 py-2' : 'rounded-2xl px-4 py-3'} ${
            isUser
              ? 'bg-gradient-to-br from-green-600 to-emerald-700 text-white'
              : 'bg-slate-800/60 backdrop-blur-lg border border-slate-700/50 text-slate-100'
          }`}>
            <div className={`whitespace-pre-wrap ${isOverlay ? 'text-sm' : ''}`}>
              {message.content}
            </div>
            ...
            {/* Timestamp */}
            <div className={`mt-2 ${isOverlay ? 'text-xs' : 'text-xs'} ${
          ...
          {!isUser && message.suggestions && message.suggestions.length > 0 && (
            <div className="space-y-2">
              <div className={`${isOverlay ? 'text-xs' : 'text-sm'} text-slate-400 font-medium`}>
                Quick actions:
              </div>
